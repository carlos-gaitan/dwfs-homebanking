...
function iniciarSesion() {
  ...
  if (currentUser) {
    ...
    if (validaPrompt(usuario)) {
      ...
      if (i > 0) {
        do {
          clave = prompt('Ingrese su CLAVE:\nUsted tendra ' + contador + ' intentos para ingresar la clave de forma correcta, de lo contario su saldo sera retenido.');
          contador--;
        } while (!validaPrompt(clave) || datosDeUsuario[i].claveUsuario != clave || contador == 0);
        if (contador > 0) {
          indiceDeUsuario = i;
